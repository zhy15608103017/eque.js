...
class EpubView extends Component {
  ...
  initBook() {
    const { url, tocChanged, epubInitOptions={openAs: "epub"} } = this.props
    if (this.book) {
      this.book.destroy()
    }
  ...
  initReader() {
    ...
    const { location, epubOptions, getRendition, setlocations } = this.props
    ...
    this.book.ready.then(() => {
      ...
      setlocations && setlocations(this.book.locations)
      // window.console.log(
      //   this.navigation,
      ...
      // )
      //生成locations对象的定位符
      this.book.locations.generate().then(i => {})
    })
